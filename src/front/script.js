//const API_BASE_URL = 'https://anaconda-uninsured-utter.ngrok-free.dev';
const API_BASE_URL = 'http://127.0.0.1:8000';

const OPERATORS = new Set(['+', '-', '*', '/']);

// The expression is the only calculation input maintained by the frontend.
const state = {
    expression: '',
    history: [],
    result: '',
...
function handleOperator(operator) {
    ...
    if (state.resultDisplayed) {
        ...
        if (operator === '-') state.expression = operator;
    } else if (OPERATORS.has(state.expression.at(-1))) {
...
function handleNegate() {
    ...
    if (!state.expression || OPERATORS.has(state.expression.at(-1))) return;
...
async function executeCalculation() {
    ...
    try {
        ...
        // Preserve the value exactly as returned by FastAPI; JavaScript never calculates it.
...
function loadHistoryResult(index) {
    ...
    state.expression = '';
    state.result = String(entry.result);
    state.resultExpression = entry.expr
        .replace(/×/g, '*')
        .replace(/÷/g, '/')
        .replace(/−/g, '-');
    state.resultDisplayed = true;
...
function handleKeypadButton(button) {
    if (button.dataset.num !== undefined) inputDigit(button.dataset.num);
    else if (button.dataset.operator !== undefined) handleOperator(button.dataset.operator);
    else if (button.dataset.action === 'clear') resetCalculator();
    else if (button.dataset.action === 'delete') handleDelete();
    else if (button.dataset.action === 'decimal') inputDecimal();
    else if (button.dataset.action === 'negate') handleNegate();
    else if (button.dataset.action === 'calculate') executeCalculation();
}
